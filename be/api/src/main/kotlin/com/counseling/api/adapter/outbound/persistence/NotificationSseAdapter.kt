...
class NotificationSseAdapter : NotificationSsePort {
    ...
    private val recipientSinks = ConcurrentHashMap<String, Sinks.Many<Notification>>()
    ...
    override fun subscribe(recipientId: String): Flux<Notification> = recipientSink(recipientId).asFlux()

    override fun removeRecipient(recipientId: String) {
        recipientSinks.remove(recipientId)?.tryEmitComplete()
    }

    private fun recipientSink(recipientId: String): Sinks.Many<Notification> =
