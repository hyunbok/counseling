...
class TenantR2dbcRepository(
        ...
        databaseClient
            .sql(
                ...
                INSERT INTO tenants (id, name, slug, status, db_host, db_port,
                                     db_name, db_username, db_password, created_at, updated_at, deleted)
                VALUES (:id, :name, :slug, :status, :dbHost, :dbPort,
                        :dbName, :dbUsername, :dbPassword, :createdAt, :updatedAt, :deleted)
    ...
    override fun findAllByDeletedFalse(): Flux<Tenant> =
        databaseClient
            .sql("SELECT * FROM tenants WHERE deleted = FALSE ORDER BY created_at")
            .map { row -> mapToTenant(row) }
            .all()
    ...
    override fun findAllByStatusAndDeletedFalse(status: String): Flux<Tenant> =
        databaseClient
            .sql(
                "SELECT * FROM tenants WHERE status = :status AND deleted = FALSE ORDER BY created_at",
            ).bind("status", status)
            .map { row -> mapToTenant(row) }
            .all()
